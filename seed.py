...
with app.app_context():
    ...
    # Ensure Chris exists
    db.execute("INSERT OR IGNORE INTO people (name) VALUES (?)", ("Chris",))
    db.commit()
    person_id = db.execute("SELECT id FROM people WHERE name = ?", ("Chris",)).fetchone()["id"]
    ...
    print(f"Seeded {count} rows for Chris from Safran")
